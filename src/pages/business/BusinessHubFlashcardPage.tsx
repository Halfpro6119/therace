...
import type { BusinessConfidenceLevel } from '../../types/businessHub';
...
export function BusinessHubFlashcardPage() {
  ...
  const handleConfidence = (level: BusinessConfidenceLevel) => {
    if (!currentTerm || !isFlipped) return;
    storage.updateBusinessFlashcardMastery(currentTerm.id, level);
    if (currentIndex < terms.length - 1) {
      setCurrentIndex(currentIndex + 1);
    } else {
      // Update topic progress for all topics that have terms in this set
