...
import { getUnseenPoetryLiteratureTasks } from '../../config/goldenEnglishQuestionBank';
import { LITERATURE_GUIDEPOST_TASK_IDS } from '../../config/englishLiteratureGuidePostData';

export function EnglishLiteratureUnseenPage() {
  const navigate = useNavigate();
  const tasks = getUnseenPoetryLiteratureTasks();
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm" style={{ color: 'rgb(var(--text-secondary))' }}>
            Analysis (Q1) and comparison (Q2) – methods, structure, Grade 9 models
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {tasks.map(task => (
          ...
          >
            ...
            >
              <Eye size={20} style={{ color: '#0EA5E9' }} />
            </div>
