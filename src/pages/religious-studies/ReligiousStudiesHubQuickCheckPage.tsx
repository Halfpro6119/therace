          ...
          {item.type === 'whichTwo' && item.options && (
            <div className="space-y-2">
              ...
              {item.options.map((opt) => {
                ...
                return (
                  <button key={opt} type="button" onClick={() => toggleWhichTwo(opt)} className={`w-full text-left rounded-lg border px-4 py-2 text-sm ${selected ? 'ring-2' : ''}`} style={{ borderColor: selected ? LAB_ACCENT : 'rgb(var(--border))', color: 'rgb(var(--text))', ...(selected ? { borderWidth: '2px' } : {}) }}>{opt}{selected ? ' ✓' : ''}</button>
                );
              })}
            </div>
          )}
