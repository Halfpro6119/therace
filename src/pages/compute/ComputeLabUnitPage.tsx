...
import { ChevronLeft, Lightbulb, BookOpen, Target, GitBranch, Calculator, CircleDot, Database, FileQuestion, ChevronRight, List, Lock, Zap } from 'lucide-react';
...
const LEARN_MODES = [
  ...
  { id: 'quick-check', title: 'Quick Check', description: 'Micro-assessments before labs', icon: Target, color: '#0891B2', order: 3 },
] as const;

const PRACTISE_MODES = [
  { id: 'algorithm-lab', title: 'Algorithm Lab', description: 'Trace tables & algorithm practice', icon: GitBranch, color: '#8B5CF6' },
  { id: 'calculation-lab', title: 'Calculation Lab', description: 'Binary, bitmap, sound, compression', icon: Calculator, color: '#10B981' },
  { id: 'logic-lab', title: 'Logic Lab', description: 'Truth tables & Boolean expressions', icon: CircleDot, color: '#F59E0B' },
  { id: 'sql-lab', title: 'SQL Lab', description: 'SELECT, INSERT, UPDATE, DELETE', icon: Database, color: '#EC4899' },
  { id: 'question-lab', title: 'Question Lab', description: 'Exam-style questions', icon: FileQuestion, color: '#6366F1' },
] as const;

export function ComputeLabUnitPage() {
  const navigate = useNavigate();
  const { unitId } = useParams<{ unitId: string }>();
  const unit = unitId ? getUnitById(unitId as ComputeUnitId) : undefined;

  const topicIds = unit ? unit.topics.map((t) => t.id) : [];
  const quickCheckSummary = unit ? storage.getComputeUnitQuickCheckSummary(unit.id, topicIds) : { passed: 0, total: 0 };
  ...
  const recommendedStep = useMemo(() => {
    ...
    if (total > 0 && passed < total) {
      return { mode: 'quick-check' as const, label: 'Complete Quick Check to unlock Algorithm Lab', path: `/compute-lab/unit/${unit.id}/quick-check` };
    }
    if (!algorithmLabUnlocked && total > 0) return { mode: 'quick-check' as const, label: 'Do Quick Check first', path: `/compute-lab/unit/${unit.id}/quick-check` };
    const all = storage.getComputeTopicProgress();
    const anyAlgorithmDone = topicIds.some((tid) => all[`${unit.id}-${tid}`]?.algorithmLabCompleted);
    if (!anyAlgorithmDone && ['3.1', '3.2'].includes(unit.id)) return { mode: 'algorithm-lab' as const, label: 'Try Algorithm Lab', path: `/compute-lab/unit/${unit.id}/algorithm-lab` };
    return { mode: 'concept' as const, label: 'Start with Concept Lab', path: `/compute-lab/unit/${unit.id}/concept` };
  }, [unit, quickCheckSummary.passed, quickCheckSummary.total, algorithmLabUnlocked, topicIds]);
  ...
  const handleMode = (modeId: string) => {
    if (modeId === 'algorithm-lab' && !algorithmLabUnlocked) return;
    navigate(`/compute-lab/unit/${unit.id}/${modeId}`);
  };
  ...
  const lockMessage = (modeId: string) => {
    if (modeId === 'algorithm-lab') return 'Complete Quick Check for at least one topic to unlock.';
    return '';
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          >
            ...
            <ChevronRight size={24} className="flex-shrink-0 ml-auto" style={{ color: 'rgb(var(--text-secondary))' }} />
          </motion.button>
          <motion.button type="button" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: 'rgb(var(--text-secondary))' }}>Exam-style questions</p>
            </div>
            <ChevronRight size={24} className="flex-shrink-0 ml-auto" style={{ color: 'rgb(var(--text-secondary))' }} />
          </motion.button>
        </div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <motion.button type="button" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            style={{ background: 'rgb(var(--surface))', borderColor: 'rgb(var(--border))' }}
          >
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: '#0891B220' }}>
      ...
      <section className="space-y-4">
        <h2 className="text-lg font-bold flex items-center gap-2" style={{ color: 'rgb(var(--text))' }}>
          <BookOpen size={20} style={{ color: LAB_ACCENT }} />
          Learn — Core path
        </h2>
        <p className="text-sm" style={{ color: 'rgb(var(--text-secondary))' }}>
          Follow this order: Concept Lab → Glossary → Quick Check → Labs
        </p>
          ...
          {LEARN_MODES.sort((a, b) => a.order - b.order).map((mode, index) => (
            ...
            >
              <div className="flex items-center gap-4 flex-1">
                ...
                <div>
                  <h3 className="text-lg font-bold mb-0.5" style={{ color: 'rgb(var(--text))' }}>{mode.title}</h3>
                  <p className="text-sm" style={{ color: 'rgb(var(--text-secondary))' }}>{mode.description}</p>
                </div>
              </div>
