    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        >
          ...
          <h1 className="text-2xl font-bold text-white mb-2">SQL Lab</h1>
          <p className="text-white/90 text-sm">No SQL tasks for this unit yet.</p>
        </motion.section>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      >
        ...
        <h1 className="text-2xl font-bold text-white mb-2">SQL Lab</h1>
        <p className="text-white/90 text-sm">SELECT, INSERT, UPDATE, DELETE</p>
      </motion.section>
      ...
      <div className="space-y-6">
        {tasks.map((task) => (
          <div key={task.id} className="rounded-xl p-6 border" style={{ background: 'rgb(var(--surface))', borderColor: 'rgb(var(--border))' }}>
            ...
            <p className="text-sm font-mono mb-2 p-3 rounded-lg bg-gray-100 dark:bg-gray-800" style={{ color: 'rgb(var(--text))' }}>Schema: {task.schema}</p>
            <p className="text-base mb-4" style={{ color: 'rgb(var(--text))' }}>{task.question}</p>
            {task.expectedQuery && (
              <details className="mt-4">
                <summary className="cursor-pointer text-sm font-medium" style={{ color: 'rgb(var(--text-secondary))' }}>Show expected query</summary>
                <pre className="mt-2 p-3 rounded-lg bg-gray-100 dark:bg-gray-800 text-sm font-mono overflow-x-auto" style={{ color: 'rgb(var(--text))' }}>
                  {task.expectedQuery}
                </pre>
              </details>
            )}
