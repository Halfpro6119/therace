...
export function ComputeLabConceptLabPage() {
  const navigate = useNavigate();
  const { unitId } = useParams<{ unitId: string }>();
  const [selectedConcept, setSelectedConcept] = useState<ComputeConcept | null>(null);
          ...
          {concepts.length === 0 && (
            <div className="rounded-xl p-6 border text-center" style={{ background: 'rgb(var(--surface))', borderColor: 'rgb(var(--border))' }}>
              ...
              <button
                type="button"
                onClick={() => navigate(`/compute-lab/unit/${unit.id}`)}
                className="text-sm font-medium px-4 py-2 rounded-lg bg-cyan-500/20 text-cyan-700 dark:text-cyan-400"
              >
                Back to Unit {unit.id}
              </button>
            </div>
        ...
        >
          ...
          >
            ...
            {selectedConcept.visualModel && (
              <div className="rounded-lg p-4 bg-cyan-500/10 border border-cyan-500/30">
            ...
            <div className="rounded-lg p-4 bg-cyan-500/10 border border-cyan-500/30 flex gap-2">
              <AlertTriangle size={20} className="text-cyan-600 dark:text-cyan-400 shrink-0 mt-0.5" />
              <div>
