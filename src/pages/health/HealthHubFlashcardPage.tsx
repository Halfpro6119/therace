...
import type { HealthUnitId, HealthConfidenceLevel } from '../../types/healthHub';
...
export function HealthHubFlashcardPage() {
  ...
  if (!unit) {
    return (
      <div className="max-w-4xl mx-auto p-8">
        ...
        <button type="button" onClick={() => navigate('/health-hub')}>Back to Health Hub</button>
      </div>
    );
  }
  ...
  const handleConfidence = (level: LabConfidenceLevel) => {
    ...
    storage.updateHealthFlashcardMastery(currentTerm.id, level as HealthConfidenceLevel);
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        >
          <div className="flex flex-col sm:flex-row gap-4">
            <button type="button" onClick={() => navigate(`/health-hub/unit/${unit.id}/quick-check`)} className="flex-1 px-6 py-3 rounded-lg font-semibold text-white bg-white/20 hover:bg-white/30">
              Quick Check →
            </button>
  ...
  if (!currentTerm) {
    return (
      <div className="max-w-4xl mx-auto p-8">
        <p style={{ color: 'rgb(var(--text))' }}>No terms for this unit yet.</p>
        <button type="button" onClick={handleBack}>Back to Unit {unit.id}</button>
      </div>
    );
  }
